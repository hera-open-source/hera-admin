...
import { getJobHistory, generateVersion, getJobVersion, manual, 
  updateSwitch, deleteJob, getJobOperator, updatePermission ,addMonitor} from '../../../../api/ScheduleCenter';
...
export default class JobForm extends Component {
  ...
  onUpdateSwitch = async (jobId) => {
    const result = await updateSwitch({ id: jobId, status: 0 });
  }
  ...
  onAddMonitor = async (jobId) => {
    const result = await addMonitor({ id: jobId });
  }
  ...
  onOk = async (name) => {
    if (name === 'manual') {
      ...
      const triggerType = this.state.triggerType;

      const result = await manual({ actionId: version, triggerType });
      this.setState({ versionVisible: false });
    } else if (name === 'permission') {
      // uIdS: ["hera"] id: 4 type: JOB
      const user = this.state.selelecedUser;
      const jobId = this.state.jobId;
      const result = await updatePermission({ names: user, id: jobId, type: 'JOB' });
  ...
  onGetJobOperator = async (jobId) => {
    const result = await getJobOperator({ jobId, type: 'JOB' });

    const data = result.data.allUser.map((item) => {
      const version = { value: item.name };
      return version;
    });
    this.setState({ allUser: data, permissionVisible: true });
  }

  onChangePermission = (jobId, user) => {
    this.setState({ selelecedUser: user, jobId });
  }
  render() {
    const { labelAlign, versionDataSource, allUser } = this.state;
      ...
      <div style={{ display: 'flex' }}>
        ...
        <div>
          <ul className="file-button-ul">
            ...
            <li>
              <Button onClick={this.onUpdateSwitch.bind(this, jobId)} size="medium" type="primary">开启/关闭</Button>&emsp;
            </li>
            <li>
              <Button onClick={this.onUpdateSwitch.bind(this, jobId)} size="medium" type="primary">失效</Button>&emsp;
            </li>
            <li>
              <Button onClick={this.onDeleteJob.bind(this, jobId)} size="medium" type="primary">删除</Button>&emsp;
            </li>
            <li>
              <Button onClick={this.onGetJobOperator.bind(this, jobId)} size="medium" type="primary">配置管理员</Button>&emsp;
            </li>
            <li>
              <Button onClick={this.onAddMonitor.bind(this, jobId)} size="medium" type="primary">关注该任务</Button>&emsp;
            </li>
          </ul>
        </div>
        ...
        <div>
          ...
          <Dialog visible={this.state.versionVisible}
            onOk={this.onOk.bind(this, 'manual')}
            onCancel={this.onClose.bind(this, 'cancelClick')}
            onClose={this.onClose}
            title="选择Job版本"
          >
            <div className="demo-container">
              <Select onChange={this.onChangeVersion.bind(this)} dataSource={versionDataSource} aria-labelledby="select-a11y" />
            </div>
          </Dialog>
          ...
          >
            <div className="demo-container">
              <Select mode="multiple" onChange={this.onChangePermission.bind(this, jobId)} dataSource={allUser} />
            </div>
          </Dialog>
        </div>
      </div >
