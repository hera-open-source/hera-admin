...
import { init, getJobMessage, getGroupMessage, getJobHistory } from '../../../../api/ScheduleCenter';
...
export default class FileOperationForm extends Component {
  ...
  onGetJobHistory = async (jobId) => {
    ...
    const result = getJobHistory({ jobId, offset: 0, pageSize: 10 });
    this.setState({ dataSource: result.data.row });
  }
  ...
  render() {
    const { jobId } = this.props || this.state;
    console.log(jobId);
    return (
      <div className="operation-button" >
        <ul className="file-button-ul">
          <li>
            <Button onClick={this.onGetJobHistory.bind(jobId, this)} size="medium" type="primary">运行日志</Button>&emsp;
          </li>
          <li>
