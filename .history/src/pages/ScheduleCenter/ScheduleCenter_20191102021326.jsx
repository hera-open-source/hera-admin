...
export default class ScheduleCenter extends Component {
  ...
  onTreeTabChange = async (activeKey) => {
    ...
    if (Object.is(activeKey, 1)) {
       ...
       = this.convertData(promise.data.myJob);
      this.setState({ treeNodes: nodes });
    } else {
      const nodes = this.convertData(promise.data.myJob);
      this.setState({ treeNodes: nodes });
    }
    this.setState({ activeKey: `${activeKey}`,treeNodes: nodes });
  }
  ...
  onSelect = async (id) => {
    ...
    if (id.toString().startsWith('group')) {
      ...
      this.setState({ folderData: result.data });
    } else {
  ...
  render() {
    const { activeKey, treeNodes, jobData, showJob, folderData } = this.state;
    ...
    return (
      <div className="schedule-center-container">
        ...
        {showJob === 1 ?
          <JobEditForm jobForm={jobData} /> : <FolderEditForm folderForm={folderData} />
        }
      </div>
    );
  }
