...
export default class ScheduleCenter extends Component {
  ...
  onJobTypeChange = async(jobRunTy) => {
    const jobId = id;
    result = await getJobMessage({ jobId });
    this.setState({ jobData: result.data, showJob: 0 });
    window.localStorage.setItem('jobId', jobId);
  ...
  render() {
    ...
    return (
      <div className="schedule-center-container">
        ...
        {showJob === 0 ?
          (editable === 0 ? <JobForm onEditJob={this.onEditJob.bind(this)} jobForm={jobData} /> : <JobEditForm onBack={this.onEditJob.bind(this)} onJobTypeChange={} jobForm={jobData} />) :
