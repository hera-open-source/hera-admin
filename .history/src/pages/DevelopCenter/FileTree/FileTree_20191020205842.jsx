...
import { init, find, addFile, deleteFile, rename } from '../../../api/DevelopCenter.js';
...
export default class FileTree extends Component {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      if (data[i].parent == pid) {
        if (data[i].isParent === false) {
          obj = { label: data[i].name, key: data[i].id.toString(), isLeaf: true };
        } else {
          obj = { label: data[i].name, key: data[i].id.toString(), isLeaf: false };
        }
        temp = this.buildTree(data, data[i].id.toString());
        if (temp.length > 0) {
          obj.children = temp;
        }
        result.push(obj);
  ...
  onEditFinish = async (key, label, node) => {
    console.log(key);
    console.log(label);
    console.log(node);
    const params = { id: `${key}', name: '${label}' };
    const promise = rename(params);
  }
