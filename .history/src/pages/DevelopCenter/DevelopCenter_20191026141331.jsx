...
import { find, debug, getLog, findDebugHistory } from '../../api/DevelopCenter.js';
...
export default class DevelopCenter extends Component {
      ...
      timerHandler = window.setInterval(() => {
        ...
        logPromise.then((logData) => {
          ...
          if (Object.is(logPanes.length, 0)) {
            ...
            this.setState({
              logPanes: tabData,
              logActiveKey: `${value.data.debugId}`,
            });
            window.localStorage.setItem('logTabData', JSON.stringify(tabData));
            window.localStorage.setItem('logActiveKey', logData.data.debugId);
  ...
  onSearchJobHistory = async (fileId) => {
    const result = await findDebugHistory({ fileId:`${fileId}` });
    this.setState({
  ...
  render() {
    let { fileActiveKey, logPanes, logActiveKey, filePanes } = this.state;
    ...
    if (Object.is(fileActiveKey, null) && !Object.is(fileActiveKeySession, null)) {
    ...
    if (Object.is(logActiveKey, null) && !Object.is(logActiveKeySession, null)) {
        ...
        <div className="code-zone">
          ...
          <div className="log-tab">
            ...
            <div>
              ...
              <Button onClick={this.onSearchJobHistory.bind(fileActiveKey, this)} size="medium" type="primary">历史日志</Button>&emsp;
