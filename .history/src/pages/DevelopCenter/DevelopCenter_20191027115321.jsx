...
import { Tree, Button, Tab, Table, Dialog, Pagination } from '@alifd/next';
...
export default class DevelopCenter extends Component {
  ...
  onFileTabChange = async (fileActiveKey) => {
     fileActiveKey = this.state.fileActiveKey || window.localStorage.getItem('fileActiveKey');
  ...
  onLogTabChange = async (logActiveKey) => {
    const logActiveKey = this.state.logActiveKey || window.localStorage.getItem('logActiveKey');
  ...
  onExecuteSelectJob = async () => {
    const editor = this.editorRefs.current.editor;
    console.log(editor.getSelection());
    const script = editor.getSelection();
    const fileActiveKey = this.state.fileActiveKey || window.localStorage.getItem('fileActiveKey');
    const promise = debug({ id: `${fileActiveKey}`, content: `${script}` });
    let timerHandler = null;
  ...
  onSearchJobHistory = async () => {
    const id = this.state.fileActiveKey || window.localStorage.getItem('fileActiveKey');
    const result = await findDebugHistory({
      fileId: id,
  ...
  operation = (value, idx, obj) => {
    console.log(obj.status);
    if (obj.status === 'running') {
      return (
        <div>
          <Button type="primary" onClick={this.onCancelJob.bind(this, obj.id)}>取消任务</Button>&nbsp;&nbsp;
        </div>
      );
    }
    return (
  ...
  expandedRowRender = (record) => {
    // const log = await getLog({ id: currentRowKey });
    if (Object.is(record.status, 'running')) {
      const timerHandler = window.setInterval(() => {
        const log = getLog({ id: record.id });
        log.then((value) => {
          console.log(value);
          if (Object.is(value.data.status, 'running')) {
            window.setInterval(timerHandler, 2000);
            return (<Scrollbars style={{ width: 800, height: 200, align: 'center' }}><div><p dangerouslySetInnerHTML={{ __html: `${value.data.log}` }} /></div></Scrollbars>);
          }
          window.clearInterval(timerHandler);
          return (<Scrollbars style={{ width: 800, height: 200, align: 'center' }}><div><p dangerouslySetInnerHTML={{ __html: `${value.data.log}` }} /></div></Scrollbars>);
        }, 2000);
      ...
      return (<Scrollbars style={{ width: 800, height: 200, align: 'center' }}><div><p dangerouslySetInnerHTML={{ __html: `${record.log}` }} /></div></Scrollbars>);
    }
  }
