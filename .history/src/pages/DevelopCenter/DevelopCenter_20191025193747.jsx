  ...
  onSelect = async (id) => {
    ...
    if (node.data.type === 2) {
      ...
      if (Object.is(panes.length, 0)) {
        ...
        const result = tabData.filter(item => Object.is(item.id, id));
        if (result.length === 0) {
          tabData.push(tab);
          this.setState({
            filePanes: tabData,
  ...
  onFileTabClose = async (targetKey) => {
    ...
    if (Object.is(panes.length, 0)) {
      ...
      if (Object.is(targetKey, fileActiveKey)) {
       ...
       let arr1 = panes.filter(item => !Object.is(item.key, targetKey));
        if (arr1.length > 0) {
          file = await find({ id: `${fileActiveKey}` });
           arr1 = panes.filter(item => !Object.is(item.key, targetKey));
          this.setState({
            filePanes: arr,
            codeMirrorValue: file.data.content,
          });
