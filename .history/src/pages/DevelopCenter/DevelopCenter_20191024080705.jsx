...
export default class DevelopCenter extends Component {
  ...
  onFileTabClose = (targetKey) => {
    ...
    if (result.length === 0) {
      ...
      window.localStorage.setItem('tabData', []);
    ...
    if (targetKey === fileActiveKey) {
      ...
      window.localStorage.setItem('tabData', result);
      localStorage.setItem('fileActiveKey', result[0].key);
    } else {
      this.setState({
        filePanes: result,
        codeMirrorValue: script,
      });
      localStorage.setItem('tabData', result);
      localStorage.setItem('fileActiveKey', fileActiveKey);
    }
  }
  ...
  onExecuteJob = () => {
    ...
    promise.then((value) => {
      ...
      localStorage.setItem('tabData', logPanes);
      localStorage.setItem('fileActiveKey', `${value.data.debugId}`);
  ...
  render() {
    ...
    const tabDataSession = localStorage.getItem('tabData');
    console.log(tabDataSession);
