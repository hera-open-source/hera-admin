...
export default class DevelopCenter extends Component {
  ...
  onSelect = (id) => {
    ...
    panes.map(item => item.closeable = false);
    panes.push(tab);
    this.setState({
      filePanes: panes,
      fileActiveKey: `${id}`,
    });
    console.log(this.state.fileActiveKey);
  }
      ...
      <div className="develop-center-container">
        ...
        <div className="code-zone">
          <div className="button-operation">
            <Button onClick={this.stateonExecuteJob} size="medium" type="primary">执行</Button>&emsp;
          ...
          <div className="file-tab">
            <Tab
              shape="wrapped"
              activeKey={this.state.fileActiveKey}
              onChange={this.onChange}
              onClose={this.onClose}
              className="custom-tab"
              size="small"
            >
              {state.filePanes.map(item => <Tab.Item title={item.tab} closeable={item.closeable}></Tab.Item>)}
            </Tab>
          </div >
          ...
          <div className="log-tab">
            <div>
              <Tab
                ...
                size="small"
              >
                {state.logPanes.map(item => <Tab.Item title={item.tab} key={item.key} closeable={item.closeable}></Tab.Item>)}
              </Tab>
            </div>
