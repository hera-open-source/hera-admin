  ...
  onExecuteJob = () => {
    ...
    promise.then((value) => {
      ...
      if (Object.is(logPanes.length, 0)) {
        const logTabDataSession = window.localStorage.getItem('logTabData');
        const logTabData = JSON.parse(logTabDataSession);
        if (Object.is(logTabData.length, 0)) {
          // 刷新页面，本身localStorage也无数据
          logPanes.push(tab);
          this.setState({
            logTabData: logPanes,
        ...
        const result = logTabData.filter(item => Object.is(item.key, id));
        if (result.length === 0) {
          tabData.push(tab);
          this.setState({
            filePanes: tabData,
