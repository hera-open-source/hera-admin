  ...
  expandedRowRender = (record) => {
    ...
    if (Object.is(record.status, 'running')) {
      const timerHandler = window.setInterval(() => {
        ...
        log.then((value) => {
          if (Object.is(value.status, 'running')) {
            console.log(value);
            window.setInterval(timerHandler, 2000);
          ...
          window.clearInterval(timerHandler);
          return (<Scrollbars style={{ width: 800, height: 200, align: 'center' }}><div><p dangerouslySetInnerHTML={{ __html: `${log.data.log}` }} /></div></Scrollbars>);
        }, 2000);
      });
    } else {
