...
export default class DevelopCenter extends Component {
  ...
  onLogTabChange = (logActiveKey) => {
    this.setState({
      logActiveKey,
    });
  }

  onLogTabClose = (logActiveKey) => {
    const panes = this.state.logPanes;
    const fileActiveKey = this.state.logActiveKey;
    const result = panes.filter(item => item.key === logActiveKey);
    let file = null;
    if (Object.is(panes.length, 0)) {
      ...
      if (!Object.is(tabDataResult.length, 0)) {
        ...
        window.localStorage.setItem('logabData', JSON.stringify(tabDataResult));
        window.localStorage.setItem('fileActiveKey', tabDataResult[0].key);
      } else if (Object.is(tabDataResult.length, 0)) {
        this.setState({
          filePanes: [],
