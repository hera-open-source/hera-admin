    ...
    if (Object.is(record.status, 'running')) {
      const timerHandler = window.setInterval(() => {
        ...
        if (Object.is(log.data.status, 'running')) {
          console.log(log);
          window.setInterval(timerHandler, 2000);
          return (<div><p dangerouslySetInnerHTML={{ __html: `${log.data.log}` }} /></div>);
        }
        window.clearInterval(timerHandler);
        return (<Scrollbars style={{ width: 1000, height: 200 }}></Scrollbars><div><p dangerouslySetInnerHTML={{ __html: `${log.data.log}` }} /></div>)</S;
      }, 2000);
    } else {
      return (<div><p dangerouslySetInnerHTML={{ __html: `${record.log}` }} /></div>);
    }
  }
