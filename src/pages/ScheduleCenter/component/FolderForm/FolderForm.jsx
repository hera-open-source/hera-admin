...
import { getGroupTask, addGroup, addJob, updatePermission } from '../../../../api/ScheduleCenter.js';
...
export default class FolderForm extends Component {
  ...
  handleJobSubmit = async (values, errors) => {
    console.log('Get form value:', values);
    const groupId = this.props.folderForm.id;
    const result = await addJob({ name: values.name, directory: values.directory, parentId: `group_${groupId}` });
    console.log(result);
    this.setState({
      addGroupVisible: false,
    });
  ...
  // id: group_2
  // type: GROUP
  handleManagerSubmit = async (values, errors) => {
    console.log('Get form value:', values);
    const groupId = this.props.folderForm.id;
    const result = await updatePermission({ name: values.name, directory: values.directory, parentId: `group_${groupId}` });
    console.log(result);
    this.setState({
      addGroupVisible: false,
    });
      ...
      <div style={{ display: 'flex' }}>
        <div>
          <Form field={this.field}>
            ...
            <FormItem
              ...
              label="配置项信息"
            >
              <Input.TextArea name="configs" value={Object.is(folderForm, undefined) ? '' : this.resolve(folderForm.configs)} />
            </FormItem>
            <FormItem
              labelAlign="top"
              label="继承的配置项信息"
            >
              <Input.TextArea name="inheritConfig" value={Object.is(folderForm, undefined) ? '' : this.resolve(folderForm.inheritConfig)} />
            </FormItem>
          </Form>
        </div>
        <div>
        ...
        <div>
          ...
          >
            ...
            <FormItem label=" ">
              <Form.Submit type="primary" onClick={this.handleJobSubmit}>添加</Form.Submit>
            </FormItem>
          </Dialog>
        </div>
        ...
        <div>
          ...
          >
            ...
            <FormItem label=" ">
              <Form.Submit type="primary" onClick={this.handleManagerSubmit}>添加</Form.Submit>
            </FormItem>
